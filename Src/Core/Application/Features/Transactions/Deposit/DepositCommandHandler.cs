...
{
    public class DepositCommandHandler : IRequestHandler<DepositCommand, ApiResponse<TransactionDto>>
    {
        ...
        public async Task<ApiResponse<TransactionDto>> Handle(DepositCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.IdempotencyKey))
            {
                ...
                if (existingTxn != null)
                {
                    return new ApiResponse<TransactionDto>(ToDto(existingTxn), "Deposit already processed (idempotent replay).");
                }
            }
            ...
            if (flagReason != null)
            {
                ...
                await _notificationService.NotifyAsync(userId, "Deposit under review",
                    $"Your deposit of {request.Amount:N2} {wallet.Currency} is under review.", cancellationToken);

                return new ApiResponse<TransactionDto>(ToDto(transaction), "Deposit is pending fraud review.");
            }
            ...
            await _notificationService.NotifyAsync(userId, "Deposit successful",
                $"Your deposit of {request.Amount:N2} {wallet.Currency} was successful.", cancellationToken);

            return new ApiResponse<TransactionDto>(ToDto(transaction), "Deposit completed successfully.");
        }
